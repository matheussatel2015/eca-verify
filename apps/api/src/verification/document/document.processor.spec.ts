...
test('verifies a document, decides aprovado, dispatches webhook, deletes both images', async () => {
  ...
  const proc = new DocumentProcessor(store, d.dataSource as any, d.verifier as any, d.audit as any, d.webhook as any, d.once as any, key, { cutoffAge: 18, margin: 3, livenessThreshold: 0.8 }, 24 * 60 * 60 * 1000, new VerificationRecordService(), null);
